...
function normalizeBlogs(rawBlogs) {
  ...
  return rawBlogs
    ...
    .map((post, idx) => ({
      ...
      date: String(post.date || 'TBD').trim() || 'TBD',
...
function loadBlogs() {
  ...
  if (!saved) {
    blogs = DEFAULT_BLOGS.map((post, idx) => ({ ...post, id: idx + 1 }));
    saveBlogs();
    return;
  }
  ...
  try {
    ...
    blogs = normalized.length ? normalized : DEFAULT_BLOGS.map((post, idx) => ({ ...post, id: idx + 1 }));
    saveBlogs();
  } catch (error) {
    blogs = DEFAULT_BLOGS.map((post, idx) => ({ ...post, id: idx + 1 }));
    saveBlogs();
