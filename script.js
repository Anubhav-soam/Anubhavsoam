...
function renderCmsStats() {
  const stats = document.getElementById('cmsStats');
  if (!stats) return;
  const published = blogs.filter((post) => post.status === 'published').length;
  const drafts = blogs.filter((post) => post.status === 'draft').length;
  stats.innerHTML = `
    <span class="cms-stat">Total: <strong>${blogs.length}</strong></span>
    <span class="cms-stat">Published: <strong>${published}</strong></span>
    <span class="cms-stat">Drafts: <strong>${drafts}</strong></span>
  `;
}
...
function exportBlogs() {
  ...
  link.download = `blogs-${new Date().toISOString().slice(0,10)}.json`;
  link.click();
  URL.revokeObjectURL(link.href);
  setEditorMessage('Blogs exported successfully.');
}

function importBlogsFromFile(file) {
  const reader = new FileReader();
  reader.onload = () => {
    try {
      const parsed = JSON.parse(String(reader.result || '[]'));
      const normalized = normalizeBlogs(parsed);
      if (!normalized.length) throw new Error('No valid blogs found');
      blogs = normalized;
      saveBlogs();
      renderTopics();
      renderBlogs();
      renderDraftList();
      renderCmsStats();
      clearEditor('Blogs imported successfully.');
    } catch {
      setEditorMessage('Import failed: invalid JSON blog format.');
    }
  };
...
function renderTopics() {
  ...
  if (!strip) return;
  const topics = ['All', ...new Set(blogs.filter((post) => post.status === 'published').map((post) => post.topic))];
...
function renderDraftList() {
  ...
  draftList.innerHTML = visible.map((post) => `
    <button type="button" class="draft-item" data-edit-draft="${post.id}">
      ${post.image ? `<img class="draft-thumb" src="${post.image}" alt="${post.title}">` : ''}
...
function renderBlogs() {
  ...
  if (!filtered.length) {
    grid.innerHTML = '<article class="blog-card blog-card-wide"><h3 class="blog-title">No published blogs yet.</h3><p>Write in the editor and publish to show posts here.</p></article>';
    return;
  }
  ...
  grid.innerHTML = filtered.map((post) => {
    ...
    return `
      <article class="blog-card blog-card-wide">
        ${post.image ? `<img class="blog-cover" src="${post.image}" alt="${post.title}">` : ''}
...
function bindImageUpload() {
  ...
  fileInput.addEventListener('change', () => {
    ...
    reader.onload = () => {
      ...
      updatePreview();
      setEditorMessage('Image uploaded and attached to this blog.');
    };
...
function initEditor() {
  ...
  ['postTopic', 'postTitle', 'postDate', 'postReadTime', 'postStatus', 'postImage', 'postExcerpt'].forEach((id) => {
  ...
  const importFile = document.getElementById('importBlogsFile');
  if (exportBtn) exportBtn.addEventListener('click', exportBlogs);
  if (importFile) {
