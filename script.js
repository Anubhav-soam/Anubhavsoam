...
function initAmbientParallax() {
  ...
  const apply = () => {
    frame = null;
    root.style.setProperty('--parallax-x', `${nextX.toFixed(2)}px`);
    root.style.setProperty('--parallax-y', `${nextY.toFixed(2)}px`);
  };

  window.addEventListener('mousemove', (event) => {
    nextX = (event.clientX - window.innerWidth / 2) * 0.018;
    nextY = (event.clientY - window.innerHeight / 2) * 0.018;
