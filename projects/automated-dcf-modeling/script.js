...
function renderWarnings(warnings) {
  ...
  return `<div class="warning-box"><strong>Model warning</strong><ul>${items}</ul></div>`;
}
...
runBtn.addEventListener("click", async () => {
  ...
  try {
    ...
    if (data.warnings?.length) {
      summaryEl.insertAdjacentHTML("afterbegin", renderWarnings(data.warnings));
    }
    renderForecastChart(data.forecast || []);
    buildForecastTable(data.forecast || []);
    sensitivityEl.innerHTML = `
      <h2>${data.sensitivity.label || "WACC / Terminal Growth Sensitivity"}</h2>
      <p class="sensitivity-note">${data.sensitivity.description || "Forecast-period FCFF remains fixed from the base case."}</p>
