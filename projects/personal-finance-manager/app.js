...
function renderProfile(){
  ...
  Plotly.newPlot('c_exp',[{type:'pie',labels:cats,values:cats.map(c=>catMap[c]),hole:.62,textfont:{color:'#dbeafe'},textinfo:'percent+label',marker:{line:{color:'#0f172a',width:2}}}],{paper_bgcolor:'rgba(0,0,0,0)',plot_bgcolor:'rgba(0,0,0,0)',font:{color:'#dbeafe'},margin:{t:30,b:20,l:20,r:20}});
  const emi = state.loanMaster.map((l,i)=>({loan:l,emi:state.loanData[i].emi})).filter(x=>x.emi>0);
  Plotly.newPlot('c_emi',[{type:'pie',labels:emi.map(x=>x.loan),values:emi.map(x=>x.emi),hole:.62,textfont:{color:'#dbeafe'},textinfo:'label+percent',marker:{line:{color:'#0f172a',width:2}}}],{paper_bgcolor:'rgba(0,0,0,0)',plot_bgcolor:'rgba(0,0,0,0)',font:{color:'#dbeafe'},margin:{t:30,b:20,l:20,r:20}});
}
